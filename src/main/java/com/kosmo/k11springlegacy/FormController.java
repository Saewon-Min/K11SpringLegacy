...
@RequestMapping
	...
	제외한 나머지 경로명으로 이루어진다. 요청명의 매핑 정보로
	메소드를 호출하게되므로 메소드명은 큰 의미가 없다.
	개발자가 구분하기 좋은 정도의 이름으로 설정하면 된다.
 */
@Controller
public class FormController {
	/*
	폼값받기 1] 파라미터로 전달되는 값을 HttpSevletRequest객체를
	...
	public String loginMember(HttpServletRequest req, Model model) {
		...
		ViewResolver가 전체 경로를 조립하여 웹브라우저에 출력한다.
		 */
	...
	폼값받기 3] 커맨드 객체를 이용해서 폼값 한번에 받기
		조건1 
			: 쿼리스트링으로 전달되는 파라미터의 개수와 폼값을 저장할
			객체(DTO 혹은 VO)의 멤버변수 개수가 동일할 때 사용할 수 있다.
		조건2
			: 커맨드 객체를 사용할때는 클래스명 앞글자를 소문자로 바꾼
			형태의 매개변수를 사용해야 한다.
			Ex ] MemberDTO -> memberDTO
		조건3
			: 커맨드 객체를 만들때 파라미터의 개수는 달라도 상관없으나
	...
	웹브라우저는 요청명을 경로로 인식하므로 리소스를 사용할떄 경로를
	주의해야 한다. 또한 파라미터의 개수가 틀려지면 404에러가 발생된다.
	 */
